...
@TRANSFORMS.register_module()
class UniformSample_Start_End(BaseTransform):
    """ UniformSample from only valid seconds
    ...
    def _get_sample_clips(self, num_frames: int) -> np.array:
        ...
        Returns:
            seq (list): the indexes of frames of sampled from the video.
        """
        # add this to handle single frame video (repeat single frame)
        if num_frames == 1:
            inds = [0] * self.clip_len # [0,0,0...]
            return np.array(inds)
    ...
    def transform(self, results: dict):
        num_frames = results['total_frames']
        start_sec  = results['start_sec']
        end_sec    = results['end_sec']
        fps        = results['avg_fps']

        start_frame_idx = int(start_sec * fps)
        end_frame_idx   = int(end_sec * fps)
        end_frame_idx   = end_frame_idx if end_frame_idx < num_frames else num_frames-1
        val_frame_idxs  = np.arange(start_frame_idx, end_frame_idx+1, 1)

        ########################################
        # below code from TrimmedUniformSample #
        ########################################
        # for valid_sec in results['valid_sec']:
        #     sec_difference  = valid_sec-start_sec
        #     val_start_frame = int(sec_difference * fps)
        #     val_end_frame   = min(int(val_start_frame + fps), num_frames)
        #     val_frame_idxs.extend(list(range(val_start_frame, val_end_frame, 1)))
        # val_frame_idxs = np.array(val_frame_idxs)

        if len(val_frame_idxs) < 1:
            val_frame_idxs = np.array([num_frames-1]) # XXX use last frame
            # raise Exception(f"[{results['sample_idx']}, {fps}, {num_frames}]{results['filename'].split('/')[-1]}, {results['valid_sec']}]: len(val_frame_idxs) == {len(val_frame_idxs)}")

        inds = self._get_sample_clips(len(val_frame_idxs))
        inds = val_frame_idxs[inds]
...
@TRANSFORMS.register_module()
class TrimmedUniformSample(BaseTransform):
    """ UniformSample from only valid seconds
