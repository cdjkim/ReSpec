...
class WebVid(BaseDataset):
    ...
    def __init__(self, config, writer, split, noise=False):
        super(WebVid, self).__init__(config, split)
    ...
    def __getitem__(self, idx):
        ...
        try:
            real_text_sim_matrix = np.array(self.sim_data[f'{int(unique_id)//100000}/{unique_id}'])
        ...
        if unique_id == -1:
            ...
            embedding = np.expand_dims(
                np.array(self.text_embed_data[f'{int(unique_id)//100000}/{unique_id}']), axis=0)
        elif self.config['modality'] == 'video':
            norm = np.linalg.norm(self.vid_embed_data[f'{int(unique_id)//100000}/{unique_id}'])
            video_emb = self.vid_embed_data[f'{int(unique_id)//100000}/{unique_id}'] / norm
            embedding = np.expand_dims(np.array(video_emb), axis=0)
        elif self.config['modality'] == 'both':
            norm = np.linalg.norm(self.vid_embed_data[f'{int(unique_id)//100000}/{unique_id}'])
            video_emb = self.vid_embed_data[f'{int(unique_id)//100000}/{unique_id}'] / norm
            concat_embedding = np.concatenate(
                [self.text_embed_data[f'{int(unique_id)//100000}/{unique_id}'], video_emb])
